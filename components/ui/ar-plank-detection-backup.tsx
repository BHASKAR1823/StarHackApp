...
export const ARPlankDetection: React.FC<ARPlankDetectionProps> = ({
  ...
  const { dimensions, deviceInfo } = useResponsiveDimensions();
  const arLayoutConfig = getARLayoutConfig();
          ...
          <View style={styles.overlay}>
            ...
            <View style={styles.arInterface}>
              <Animated.View style={[
                ...
                pulseAnimatedStyle,
                { top: arLayoutConfig.poseIndicatorTop }
              ]}>
                <View style={[
                  styles.poseCircle,
                  { 
                    width: arLayoutConfig.poseIndicatorSize,
                    height: arLayoutConfig.poseIndicatorSize,
                    borderRadius: arLayoutConfig.poseIndicatorSize / 2,
                    ...
                    shadowOpacity: 0.8,
                    shadowRadius: deviceInfo.isTablet ? 15 : 10,
                    elevation: 20
                  }
                ]}>
                  <View style={[
                    ...
                    { 
                      width: arLayoutConfig.poseIndicatorSize * 0.75,
                      height: arLayoutConfig.poseIndicatorSize * 0.75,
                      borderRadius: (arLayoutConfig.poseIndicatorSize * 0.75) / 2,
                      backgroundColor: isInPlank ? '#4CAF50' : '#FF5722' 
                    }
                  ]}>
                    <IconSymbol 
                      name={isInPlank ? "checkmark" : "figure.walk"} 
                      size={deviceInfo.isTablet ? 40 : deviceInfo.screenType === 'small' ? 24 : 32} 
                      color="white" 
                    />
                  </View>
                </View>
              ...
              <View style={styles.arGrid}>
                {Array.from({ length: arLayoutConfig.gridSpacing }).map((_, i) => (
                  <View
                    key={i}
                    style={[
                      styles.gridLine,
                      {
                        opacity: isInPlank ? arLayoutConfig.gridOpacity + 0.4 : arLayoutConfig.gridOpacity,
                        backgroundColor: isInPlank ? '#4CAF50' : '#FFF',
                        top: (i * (dimensions.height / arLayoutConfig.gridSpacing)) + hp(12)
                      }
                    ]}
                  />
                ))}
                {Array.from({ length: Math.floor(arLayoutConfig.gridSpacing * 0.8) }).map((_, i) => (
                  <View
                    key={`v-${i}`}
                    style={[
                      styles.gridLineVertical,
                      {
                        opacity: isInPlank ? arLayoutConfig.gridOpacity + 0.4 : arLayoutConfig.gridOpacity,
                        backgroundColor: isInPlank ? '#4CAF50' : '#FFF',
                        left: i * (dimensions.width / Math.floor(arLayoutConfig.gridSpacing * 0.8))
                      }
                    ]}
                  />
                ))}
            ...
            <Animated.View style={[
              styles.confidenceHUD, 
              confidenceAnimatedStyle,
              {
                top: arLayoutConfig.confidenceHUDTop,
                right: arLayoutConfig.confidenceHUDRight,
                width: arLayoutConfig.confidenceHUDWidth
              }
            ]}>
            ...
            {!showCountdown && (
              <View style={[
                styles.arTimerDisplay,
                {
                  bottom: arLayoutConfig.timerDisplayBottom,
                  width: arLayoutConfig.timerDisplayWidth,
                  alignSelf: 'center'
                }
              ]}>
...
const createStyles = () => {
  const { isTablet, screenType } = getDeviceInfo();
  const { top: safeAreaTop } = getSafeAreaAdjustments();
  ...
  return StyleSheet.create({
    ...
    closeButton: {
      ...
      borderRadius: rs(20),
      minWidth: rs(44), // Ensure touch target size
      minHeight: rs(44),
      alignItems: 'center',
    ...
    poseIndicator: {
      ...
      position: 'absolute',
      alignSelf: 'center',
      left: wp(50) - rs(50), // Center horizontally
    },
    poseCircle: {
      alignItems: 'center',
      justifyContent: 'center',
